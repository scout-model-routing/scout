"""
SCOUT (no decouple) many-scanners: method selection without normalization
constant estimation, with multiple pass/scanner columns.

Appends num_scanners pass columns to scores before splitting, trains a softmax
neural network on category-enhanced soft labels, then evaluates regret by
slicing the pass columns back out.
"""
...
from scout_proper import (
    single_fold_evaluation, _validate_arrays, _run_and_save_regret_exps, METHODS,
)
...
def main():
    parser = argparse.ArgumentParser(
        description='SCOUT (no decouple) many-scanners evaluation script'
    )
    ...
    folder_prefix = 'scout_no_decouple_many_scanners'
    results_path = f'{results_folder}/{folder_prefix}/results{args.folder_suffix}'
    os.makedirs(results_path, exist_ok=True)

    # Load data and append pass columns before splitting
    data = np.load(data_path)
    y_all = data['y_all']
    y_all = np.hstack([y_all, _build_pass_columns(y_all.shape[0], num_scanners)])
    ...
    if num_scanners > 0:
        utility_test_predictions = utility_test_predictions[:, :-num_scanners]
        utility_y_test = utility_y_test[:, :-num_scanners]
        utility_y_train_val = utility_y_train_val[:, :-num_scanners]
        utility_train_val_predictions = utility_train_val_predictions[:, :-num_scanners]
